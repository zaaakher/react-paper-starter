...
class Drawing extends React.Component {
  componentDidMount() {
    ...
    // bg.fillColor = "red";
    let divising = 20;
    this.makeGrid(window.innerWidth / divising, window.innerHeight / divising);
    // this.makeGrid(10,10);
    let circleGroup = Paper.project.activeLayer.children[0];

    circleGroup.children.map((cell, i) => {
      let path = new Paper.Path.Circle(cell.bounds.center, 5);
      ...
      let n = 35;
      path.rotate(n * i);
      let cir = new Paper.Path.Circle(path.getPointAt(path.length - 1), 2);
      cir.fillColor = "white";
